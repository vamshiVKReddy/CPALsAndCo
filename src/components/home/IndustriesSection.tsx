...
const industries = [
  {
    name: "Manufacturing",
    desc: "Production & industrial units",
    icon: (
  ...
  {
    name: "Real Estate",
    desc: "Developers & property firms",
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
  ...
  {
    name: "Retail & FMCG",
    desc: "Retail chains & distributors",
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
  ...
  },
  {
    ...
    desc: "Consulting & advisory firms",
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" aria-hidden="true">
...
export function IndustriesSection() {
  return (
    <section className="py-24 lg:py-32 bg-slate-900 text-white" aria-labelledby="industries-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <FadeIn>
            <SectionLabel light>Sector Coverage</SectionLabel>
          </FadeIn>
          <FadeIn delay={0.06}>
            <h2
              id="industries-heading"
              className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white tracking-tight mb-4 mt-1"
            >
              Industries We Serve
            </h2>
          </FadeIn>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {industries.map((industry, i) => (
            <FadeIn key={industry.name} delay={0.04 + (i % 5) * 0.04}>
              <div className="group p-5 rounded-2xl bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] hover:border-blue-500/30 transition-all duration-300 text-center cursor-default">
                <div className="w-11 h-11 mx-auto mb-3 rounded-xl bg-blue-600/15 border border-blue-500/20 text-blue-400 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all duration-300">
                  {industry.icon}
                </div>
                <h3 className="font-semibold text-white text-sm leading-tight mb-1">
                  {industry.name}
                </h3>
                <p className="text-slate-500 text-xs leading-tight">{industry.desc}</p>
              </div>
            </FadeIn>
          ))}
        </div>
